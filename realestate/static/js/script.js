...
document.addEventListener("DOMContentLoaded", function () {
	...
	city.addEventListener("change", function () {
		fetch(`/ajax/load-areas/?city_id=${this.value}`)
			.then(res => res.json())
			.then(data => {
				area.innerHTML = '<option value="">All Areas</option>';
				subarea.innerHTML = '<option value="">All SubAreas</option>';

				data.forEach(a => {
					area.innerHTML += `<option value="${a.id}">${a.name}</option>`;
				});
	...
	area.addEventListener("change", function () {
		fetch(`/ajax/load-subareas/?area_id=${this.value}`)
			.then(res => res.json())
			.then(data => {
				subarea.innerHTML = '<option value="">All SubAreas</option>';

				data.forEach(s => {
					subarea.innerHTML += `<option value="${s.id}">${s.name}</option>`;
				});
